...
var stats = {
  ...
  proc_calc: function(pid, options, done) {
    ...
    fs.readFile(p.join('/proc', ''+pid, 'stat'), 'utf8', function(err, infos) {
      ...
      var total = 0, user=0, system=0;

      if(history.utime) {
        system = stat.stime - history.stime;
        user = stat.utime - history.utime;
      } else {
        system = stat.stime;
        user = stat.utime;
      }
      total = system + user + childrens;

      total = total / cpu.clock_tick;
      system = system / cpu.clock_tick;
      user = user / cpu.clock_tick;
      ...
      return done(null, {
        ...
        system: (system / seconds) * 100,
        user: (user / seconds) * 100,
  ...
  win: function(pid, options, done) {
    var history = this.history[pid] ? this.history[pid] : {}, self = this;
      ...
      wmic.on('close', function () {
        var uptime = getHrTimeNanos();
        stdout = stdout.split(os.EOL)[1];
        stdout = stdout.replace(/\s\s+/g, ' ').split(' ');

        // according to http://technet.microsoft.com/en-us/library/ee176718.aspx 
        // values are in milliseconds
        // according to http://blogs.technet.com/b/heyscriptingguy/archive/2005/09/22/how-can-i-monitor-the-activity-level-of-a-process.aspx 
        // values are in 100s of nanos... WTF?
        // according to https://msdn.microsoft.com/en-us/library/windows/desktop/aa394372(v=vs.85).aspx
        // says values are in milliseconds in one place AND values are in 100 nanos in another place
        // 
        // empircal evidence on win2008 server points to 100s of nanos
        var stat = {
            kernelmodetime: parseFloat(stdout[0]) * 100,
            usermodetime: parseFloat(stdout[1]) * 100,
            workingsetsize: parseFloat(stdout[2]) * 100
        };

        
        var total = 0, user=0, system=0;
        if(history.usermodetime) {
          system = stat.kernelmodetime - history.kernelmodetime;
          user = stat.usermodetime - history.usermodetime;
        } else {
          // don't have anything sensible for the first time, just allow to be zero
        }
        total = system+user;

        //time elapsed between calls
        var nanos = history.uptime !== undefined ? uptime - history.uptime : 1; 
        nanos = Math.abs(nanos)
        nanos = nanos === 0 ? 0.1 : nanos //we sure can't divide through 0

        self.history[pid] = stat
        self.history[pid].nanos = nanos
        self.history[pid].uptime = uptime

        return done(null, {cpu: (total/nanos)*100, system:(system/nanos)*100, user:(user/nanos)*100, memory: stat.workingsetsize});
      });
